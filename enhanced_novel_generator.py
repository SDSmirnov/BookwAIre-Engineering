...
"""
    ...
    def generate_novel(self, num_chapters):
        ...
        for i, chapter_scenes in enumerate(scene_chunks):
            ...
            {self.world_bible}
            """
            critique = self._call_gemini(prompt_4_1, temperature=0.6, top_p=0.8, top_k=25)
            ...
            ### КЛЮЧЕВЫЕ ПРОБЛЕМЫ ИИ-ТЕКСТОВ (что исправлять):

            1. **ИЗБЫТОЧНАЯ ДЕТАЛИЗАЦИЯ:** Удали описания очевидных действий, оставь пространство для воображения
            2. **ГИПЕРТРОФИРОВАННАЯ МЕТАФОРИЧНОСТЬ:** Максимум 1-2 метафоры на главу, замени пустые эпитеты простыми словами
            3. **ЭМОЦИОНАЛЬНАЯ ПЕРЕГРУЖЕННОСТЬ:** Убери лишние прилагательные типа "ледяной", "жуткий", "зловещий"
            4. **НЕЕСТЕСТВЕННЫЕ ДИАЛОГИ:** Добавь междометия, паузы, незаконченные фразы
            5. **МЕХАНИСТИЧНОСТЬ:** Разнообразь длину предложений, убери шаблонные переходы
            ...
            **РЕЗЮМЕ ГЛАВЫ {chapter_num}:**
            ...
            {edited_chapter}
            """
            response = self._call_gemini(prompt_4_3, temperature=0.5, top_p=0.8, top_k=20, use_world_model=True)ЕВЫЕ ПРОБЛЕМЫ ИИ-ТЕКСТОВ (что исправлять в первую очередь):
            ...
            **РЕЗЮМЕ ГЛАВЫ {chapter_num}:**
            ...
            {edited_chapter}
            """
            response = self._call_gemini(prompt_4_3, temperature=0.5, top_p=0.8, top_k=20)
